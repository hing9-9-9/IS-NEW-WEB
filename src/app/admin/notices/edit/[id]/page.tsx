...
import { ArrowLeft, Save, Paperclip, X, Download } from 'lucide-react';
import { noticesApi, NoticeAttachment } from '@/lib/api';
...
export default function EditNoticePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchNotice = async () => {
      try {
        const notice = await noticesApi.getById(id);
        setFormData({
          title: notice.title,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (pendingFiles.length > 0) {
        await noticesApi.uploadAttachments(id, pendingFiles);
      }
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1">공지사항을 수정합니다</p>
        </div>
      </div>
        ...
        <div>
          ...
          {attachments.length > 0 && (
            <ul className="mb-3 space-y-2">
              {attachments.map((att) => (
                <li key={att._id} className="flex items-center justify-between bg-blue-50 rounded-lg px-3 py-2 text-sm">
                  <a
                    href={`${API_URL}${att.path}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 text-blue-700 hover:underline truncate max-w-xs"
                  >
                    <Download size={14} />
                    {att.originalName}
                  </a>
